 ...
 * nativePopup
 * 접근한 사용자 디바이스를 체크하여
 * 웹일경우 팝업창을, 앱일경우 네이티브웹을 띄운다.
 *
 * 별도로 생활관일 경우 암호화된 파라미터값(enc)을 서버에서 가져온 후 페이지를 이동한다.
 *
 * */
...
/**
 * bindParams 해당객체에 파라미터정보를 바인딩한다.
 */
	...
	$.fn.layout = function ( params ) {
		...
		$( this ).show().dialog( $.extend( {
			...
			"create" : function () {
				...
				$( ".ui-dialog" ).css( "top" ,"0" );
				$( this ).find( "a" ).not( ".FILE_DOWNLOAD,.LAYOUT_CLOSE_BTN,.ui-tabs-anchor" ).each( function () {
					if ( !$( this ).attr( "href" ).match( /^(\#|javascript\:)(.+)/gi ) ) {
